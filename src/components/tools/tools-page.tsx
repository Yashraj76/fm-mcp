...
export function ToolsPage() {
  ...
  // Already known from this page's own server list — hand it to ToolDialog so
  // it doesn't have to wait on its own server-detail fetch to start loading
  // the compiled schema.
  const selectedServerConnectionId = selectedServer?.connections?.[0]?.connection?.id ?? null

  // Tool rows already carry their own handlerConfig — prefer that connectionId
  // over the server default when editing an existing tool.
  const getToolConnectionId = (tool: Record<string, unknown>): string | null => {
    const handlerConfig = typeof tool.handlerConfig === 'string'
      ? safeParseJSON<Record<string, unknown>>(tool.handlerConfig, {})
      : (tool.handlerConfig as Record<string, unknown> | undefined) ?? {}
    return (handlerConfig?.connectionId as string | undefined) ?? selectedServerConnectionId
  }
  ...
  // Delete mutation. The branch-scoped DELETE route explicitly rejects
  ...
  const isOnMainBranch = !currentBranchId || !!selectedServer?.branches?.find((b: any) => b.id === currentBranchId)?.isDefault
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 mb-6 border-b pb-5">
        ...
        <div className="flex items-center gap-2 w-full xl:w-auto xl:justify-end">
          ...
          <Button
            size="sm"
            onClick={() => setShowToolDialog(true, null, selectedServerConnectionId)}
            className="gap-1.5 flex-1 xl:flex-initial"
            disabled={!currentServerId}
          >
      ...
      {isError ? (
        <Card className="border-destructive/30 bg-destructive/5">
          <CardContent className="p-6 text-center">
            ...
            <Button
              ...
              size="sm"
              className="mt-3"
              onClick={() => invalidateToolLists(queryClient, currentServerId, currentBranchId)}
            >
              Retry
            </Button>
          </CardContent>
        ...
        <EmptyState
          ...
          onCreateNew={() => setShowToolDialog(true, null, selectedServerConnectionId)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
          {(filteredTools as any[]).map((tool: any) => (
            <ToolCard
              ...
              tool={tool}
              onEdit={() => setShowToolDialog(true, tool.id as string, getToolConnectionId(tool))}
      ...
      {/* ToolDialog is mounted once in GlobalDialogs (protected layout) —
          do not also render it here or two dialogs stack with duplicate ids. */}
