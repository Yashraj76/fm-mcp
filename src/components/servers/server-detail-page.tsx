...
function ToolRow({ tool, onToggle, readOnly }: { tool: ToolItem; onToggle?: () => void; readOnly?: boolean }) {
  return (
...
export function ServerDetailPage() {
  ...
  const { data: server, isLoading, isError } = useQuery<ServerDetail>({
    ...
    queryFn: () => fetch(`/api/servers/${currentServerId}`).then(r => r.json()),
  ...
  const activeTools = defaultBranch?.tools.filter(t => t.isEnabled) || []
            ...
            <div className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <Button variant="outline" className="w-full justify-start" size="sm" onClick={() => setShowConfigDialog(true)}>
                    <FileJson className="size-3.5" />
                    Generate Configuration
                  </Button>
                  <Button variant="outline" className="w-full justify-start" size="sm" onClick={() => setShowAiDialog(true)}>
                    <Sparkles className="size-3.5" />
              ...
              <CardContent className="max-h-96 overflow-y-auto">
                {lastDeployment ? (
                  <>
                    ...
                    <div className="divide-y">
                      {defaultBranch?.tools.filter(t => t.isEnabled).map(tool => (
                        <ToolRow key={tool.id} tool={tool} readOnly />
                      ))}
                    </div>
                  </>
