...
function CodeBlock({ code, label }: { code: string; label?: string }) {
  return (
    <div className="relative group">
      ...
      <div className="bg-muted rounded-lg p-4 font-mono text-xs overflow-x-auto whitespace-pre border max-h-48 overflow-y-auto leading-relaxed">
        <code>{code}</code>
      </div>
    </div>
  )
...
function RevealableToken({ value }: { value: string }) {
  ...
  return (
    <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-2 border font-mono text-sm">
      ...
      <button onClick={() => setShown(s => !s)} className="text-muted-foreground hover:text-foreground transition-colors shrink-0" aria-label={shown ? "Hide token" : "Show token"}>
  ...
  return (
    <Dialog open={showConfigDialog} onOpenChange={(open) => { setShowConfigDialog(open); if (!open) setNewApiKey(null) }}>
      <DialogContent className="sm:max-w-2xl max-h-[92vh] overflow-y-auto p-0">
        <DialogHeader className="p-6 pb-0 sticky top-0 bg-background z-10 border-b pb-4">
          ...
          <DialogDescription className="text-muted-foreground mt-1 text-sm">
            {config ? (
              <span>
                <span className="text-foreground font-medium">{config.serverName}</span>
            ...
            <>
              ...
              <div className="space-y-3 rounded-xl border p-4">
                ...
                {apiKeyMeta && !newApiKey && (
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    <span>Prefix: <code className="font-mono text-foreground/80">{apiKeyMeta.keyPrefix}…</code></span>
                    <span>·</span>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-foreground">Client Configuration</span>
                </div>
                ...
                <Tabs defaultValue="streamable">
                  <TabsList className="h-9">
                    <TabsTrigger
                      value="streamable"
                      className="data-[state=active]:bg-accent text-muted-foreground data-[state=active]:text-foreground text-xs gap-1.5"
                    >
                  ...
                  <TabsContent value="streamable" className="space-y-3 mt-4">
                    <div className="text-xs text-muted-foreground space-y-0.5">
                      <p className="font-medium text-foreground">Cursor · VS Code · ChatGPT · Claude Code</p>
                    ...
                    />
                    <div className="bg-muted rounded-lg border px-3 py-2 text-xs text-muted-foreground">
                  ...
                  <TabsContent value="remote" className="space-y-3 mt-4">
                    <div className="text-xs text-muted-foreground space-y-0.5">
                      <p className="font-medium text-foreground">Any stdio client (Claude Desktop without native HTTP)</p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="divide-y rounded-lg border overflow-hidden">
                    {(config?.tools ?? []).slice(0, 10).map((tool) => (
                      <div key={tool.name} className="flex items-start gap-3 px-3 py-2.5 hover:bg-muted/50 transition-colors">
